      ...
      <v-card>
        ...
        >
          ...
          <template #item.channel="{ item }">
            {{ item.channel || defaultChannel || '默认' }}
          </template>
...
export default {
  ...
  methods: {
    apiRoot() {
      return "/api/v1/plugin/rsshub_reader";
    },
    imgUrl(raw) {
      if (!raw) return "";
      if (!this.useProxy) return raw;
      return this.apiRoot() + "/proxy?url=" + encodeURIComponent(raw);
    },
    // 统一 API 调用（自动处理错误提示）
    ...
    async addFeed() {
      ...
      try {
        const data = await this.call("/feeds", {
          method: "POST",
          body: JSON.stringify({ url: this.newUrl.trim(), name: this.newName.trim() }),
        });
        if (data.ok) {
          this.feeds = data.feeds;
          this.newUrl = "";
    ...
    async toggleRead(art) {
      ...
      art.is_read = !wasRead;
      this.$forceUpdate();
      const data = await this.call("/read", {
    ...
    async downloadOpml() {
      ...
      a.href = this.apiRoot() + "/opml/export";
      a.target = "_blank";
      a.rel = "noopener";
    ...
    async loadConfig() {
      // 读取全局配置（默认通知渠道等），供下拉默认值使用
      try {
        const res = await fetch(this.apiRoot() + "/routes");
        // 配置通过 MP 表单持久化，前端仅读取默认值；失败时不影响功能
      } catch (e) {}
    },
    async loadRules() {
    ...
    async deleteRule(rule) {
      ...
      const data = await this.call("/rules?url=" + encodeURIComponent(rule.id), {
        method: "DELETE",
      });
      if (data.ok) {
        this.rules = data.rules || [];
    ...
    // notifyEnabled 开关写入后台（复用配置：通过 rules 的 enabled 字段控制全局）
    // 这里采用简化策略：关闭时把所有规则 enabled=false，开启时恢复——由后端 notify_enabled 配置决定
    // 前端仅做本地提示，实际生效以 MP 配置表单「启用规则通知」为准
  },
  ...
  computed: {
    ...
    channelItems() {
      return [
        ...
        { title: "ServerChan", value: "ServerChan" },
        { title: "PushPlus", value: "PushPlus" },
        { title: "企业微信", value: "WeChatWork" },
